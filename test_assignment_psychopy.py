...
# no testing of quit_function and bias_function because they simply return variables or functions unless some error is triggered.
...
def test_stick_to_prev_com_choice_function ():
    ...
    if prev_com_choice == "h":
        case_heads = stick_to_prev_com_choice_function (prev_com_choice, cut_off, bias)
        assert case_heads == 0.9
    else:
        case_tails = stick_to_prev_com_choice_function (prev_com_choice, cut_off, bias)
...
def test_switch_from_prev_com_choice_function ():
    prev_com_choice = random.choice(['h', 't'])
    if prev_com_choice == "h":
        case_heads = switch_from_prev_com_choice_function(prev_com_choice, cut_off, bias)
        assert case_heads == 0.1
    else:
        case_tails = switch_from_prev_com_choice_function(prev_com_choice, cut_off, bias)
...
def test_bias_stick_to_prev_user_choice_function ():
    ...
    if prev_subj_choice == "h":
        case_heads = bias_stick_to_prev_user_choice_function(prev_subj_choice, cut_off, bias)
        assert case_heads == 0.9
    else:
        case_tails = bias_stick_to_prev_user_choice_function(prev_subj_choice, cut_off, bias)
        assert case_tails == 0.1
...
def test_bias_switch_from_prev_user_choice_function ():
    prev_subj_choice = random.choice(['h', 't'])
    if prev_subj_choice == "h":
        case_heads = bias_switch_from_prev_user_choice_function(prev_subj_choice, cut_off, bias)
        assert case_heads == 0.1
    else:
        case_tails = bias_switch_from_prev_user_choice_function(prev_subj_choice, cut_off, bias)
        assert case_tails == 0.9
